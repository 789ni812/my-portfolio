...
import { DevVsPM as data } from './data-points';
...
const formatInfo = (target) => {
    ...
    const companyName = data[point].companyName
    // console.log('data point: ', data[point].companyName);
    const argument = data[point].year;
    return `${series} ${value} Company: ${companyName}`;
};
...
const encodeTarget = ({ series, point }) => (2 * point + Number(series === 'Developer'));
const decodeTarget = code => ({ series: code % 2 ? 'Developer' : 'PM', point: Math.floor(code / 2) });
